...
function startServerProcess(project) {
  ...
  const lakePath = process.env.LAKE_PATH || '/home/exedev/.elan/bin/lake';
  console.log(`Lake path: ${lakePath}`);
  console.log(`Current PATH: ${process.env.PATH}`);
  const spawnOpts = { cwd: projectPath, env: { ...process.env, PATH: process.env.PATH || '/home/exedev/.elan/bin:/usr/local/bin:/usr/bin:/bin' } };
  let serverProcess;
  if (isDevelopment) {
    serverProcess = cp.spawn(lakePath, ["serve", "--"], spawnOpts);
  } else {
    if (hasWorkingBwrap()) {
