...
def impossible_stairs(start, P, scale=1, base=0, rail=True):
    ...
    if rail:
        for i,(p,q,d) in enumerate(zip(landings,landings[1:],dirs)):
            ...
            for side in (-1,1):
                ...
                a=p+lateral*offset+lift; b=a+d*width/2
                start_rail=rod('Landing rail return',a,b,.015,P['bronze'])
                if i==0:first.append(start_rail)
                if i==3:last.append(start_rail)
                a=q-d*width/2+lateral*offset+lift
                b=q+lateral*offset+lift; elbow=q+(lateral+next_lat)*offset+lift
                c=q+next_lat*offset+lift
                rail_parts=[rod('Landing rail return',a,b,.015,P['bronze']),rod('Continuous corner railing',b,elbow,.015,P['bronze']),rod('Continuous corner railing',elbow,c,.015,P['bronze']),rod('Corner baluster',elbow-lift,elbow,.011,P['bronze'])]
