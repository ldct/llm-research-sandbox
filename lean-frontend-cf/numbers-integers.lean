...
lemma zero_ne_one : (0 : MyInt) ≠ 1 := by
  rw [zero_def, one_def, Ne, Quotient.eq]
  simp [Setoid.r, R]
...
lemma mul_ne_zero (x y : MyInt) : x ≠ 0 → y ≠ 0 → x * y ≠ 0 := by
  ...
  simp only [Quot_eq_Quotient] at h1 h2 ⊢
  rw [zero_def] at h1 h2 ⊢
  rw [mul_def]
  rw [Ne, Quotient.eq] at h1 h2 ⊢
  simp [Setoid.r, R] at h1 h2 ⊢
  intro h
...
lemma i_injective : Function.Injective i := by
  intro a b h
  simp [i, Quotient.eq, Setoid.r, R] at h
  linarith
...
lemma add_le_add_right (x y : MyInt) (h : x ≤ y) (z : MyInt) : x + z ≤ y + z := by
  rcases h with ⟨n, rfl⟩
  use n
  ring
...
lemma mul_pos (x y : MyInt) (hx : 0 < x) (hy : 0 < y) : 0 < x * y := by
  have hx_le := hx.1
  have hy_le := hy.1
  have hx_ne : x ≠ 0 := by intro h; exact hx.2 (h ▸ le_refl 0)
  have hy_ne : y ≠ 0 := by intro h; exact hy.2 (h ▸ le_refl 0)
  have h_le : 0 ≤ x * y := by
    rcases hx_le with ⟨n, rfl⟩
    rcases hy_le with ⟨m, rfl⟩
    simp
    use n * m
    rw [i_mul]
    ring
  ...
  · exact h_le
  · intro h
    exact mul_ne_zero x y hx_ne hy_ne (le_antisymm _ _ h h_le)
